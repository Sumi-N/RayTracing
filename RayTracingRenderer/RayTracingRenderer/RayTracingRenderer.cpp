...
{	
	...
	for (auto light = lights.begin(); light != lights.end(); ++light)
	{
		if (strcmp((*light)->GetName(), "directLight") == 0)
		{
			...
			{
				IR = (*light)->Illuminate(hInfo.p, N);
			}

			Color diffusepart = diffuse.Sample(hInfo.uvw, hInfo.duvw) * N.Dot(L);
			specularpart = powf(H.Dot(hInfo.N), glossiness) * specular.Sample(hInfo.uvw, hInfo.duvw);
		...
		else if (strcmp((*light)->GetName(), "pointLight") == 0)
		{
			...
			diffusepart = diffuse.Sample(hInfo.uvw, hInfo.duvw) * N.Dot(L);
			specularpart = powf(H.Dot(hInfo.N), glossiness) * specular.Sample(hInfo.uvw, hInfo.duvw);
			ClampColorValue(specularpart);
			color += (diffusepart + specularpart) * IR;
		}
	...
	if (bounce < GIBOUNCE)
	{
		...
		if (rand < borderline)
		{
			...
			if (diffuse.Sample(hInfo.uvw) != Color(0, 0, 0))
			{
				...
				returnColor = 1/borderline * diffuse.Sample(hInfo.uvw, hInfo.duvw) * GlobalIlluminationTraverse(ray_gi, bouncetime);
			}
		}
		else
		{
			...
			if (specular.Sample(hInfo.uvw) != Color(0, 0, 0))
			{
				...
				returnColor = 1 / (1 - borderline) * specular.Sample(hInfo.uvw, hInfo.duvw) * GlobalIlluminationTraverse(ray_gi, bouncetime);
			}
		}
